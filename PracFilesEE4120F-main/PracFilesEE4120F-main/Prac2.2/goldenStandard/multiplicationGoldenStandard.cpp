...
int main(void){
	...
	int i, j, k;
	for(i=0; i<Size; i++){//Equivalent to group size
		for(j=0; j<Size; j++){//Equivalent to local size
			output[j+i*Size] = 0; //Index equivalent to global size (GroupSize*Size + LocalSize)
			for(k=0; k<Size; k++){//Summation of elements
				output[j+i*Size] += matrixA[k+i*Size]*matrixB[j+k*Size];
			}
		}
	}
