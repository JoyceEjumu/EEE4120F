...
int main(void)
{
	...
	cl_mem matrixA_buffer, matrixB_buffer, output_buffer, matrixSize_buffer;
	...
	size_t global_size = Size*Size; //Total Number of Work Iterms - Might change this later
	size_t local_size = Size; //Total Number of Work Groups - Might change this later
	...
	int output[global_size]; //To determine Array Size later Once determined number of work groups and work items
	...
	matrixSize_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(int), &Size, &err);
	...
	clSetKernelArg(kernel, 2, sizeof(cl_mem), &matrixSize_buffer);
	clSetKernelArg(kernel, 3, sizeof(cl_mem), &output_buffer);
	clSetKernelArg(kernel, 4, (local_size+global_size)*sizeof(int), NULL); //Consider editing if need be
	//------------------------------------------------------------------------
